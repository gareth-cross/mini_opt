...
static void CheckParams(const ConstrainedNonlinearLeastSquares::Params& params) {
  ...
  F_ASSERT_GE(params.lambda_successful_restore_decrease, 0);
  F_ASSERT_LT(params.lambda_successful_restore_decrease, 1.0);
}
...
NLSSolverOutputs ConstrainedNonlinearLeastSquares::Solve(const Params& params,
  ...
  for (int iter = 0; iter < params.max_iterations; ++iter) {
    ...
    if (step_result == StepSizeSelectionResult::SUCCESS) {
      penalty = MaybeReducePenalty(errors_pre, directional_derivative, penalty);
    }
...
NLSTerminationState ConstrainedNonlinearLeastSquares::UpdateLambdaAndCheckExitConditions(
  ...
  if (step_result == StepSizeSelectionResult::SUCCESS) {
    ...
    state_ = OptimizerState::NOMINAL;
    lambda = std::max(lambda * params.lambda_successful_restore_decrease, params.min_lambda);
...
StepSizeSelectionResult ConstrainedNonlinearLeastSquares::SelectStepSize(
  ...
  for (int iter = 0; iter <= max_iterations; ++iter) {
    ...
    if (strategy == LineSearchStrategy::POLYNOMIAL_APPROXIMATION) {
      ...
      F_ASSERT(strategy == LineSearchStrategy::ARMIJO_BACKTRACK);
      if (iter > 0) {
...
double ConstrainedNonlinearLeastSquares::MaybeReducePenalty(
    const Errors& previous_errors, const DirectionalDerivatives& derivatives,
    const double current_penalty) const {
  F_ASSERT(!steps_.empty());
  const LineSearchStep& accepted_step = steps_.back();

  const double actual_reduction =
      previous_errors.Total(current_penalty) - accepted_step.errors.Total(current_penalty);
  const double predicted_reduction = -derivatives.Total(current_penalty) * accepted_step.alpha;

  if (actual_reduction / predicted_reduction > 0.75) {
    return std::max(current_penalty / 1.5, 1.0);
  }

...
//       Eigen::Matrix<double, ResidualDim, NumParams>
//           J;
...
template <typename F, typename... ArgsValues>
static void TestResidualFunctionDerivative(double tol, F function, ArgsValues... values) {
  using traits = function_traits<F>;
  using return_type = typename traits::return_type;
  constexpr int residual_dim = return_type::RowsAtCompileTime;

  static_assert(traits::arity == sizeof...(ArgsValues) * 2);

  using param_types = type_list_take_n_t<sizeof...(ArgsValues), typename traits::args_list>;
  using intrinsic_param_types = type_list_map_t<std::remove_cvref_t, param_types>;

  function_evaluator<residual_dim, intrinsic_param_types> eval{};
  eval.compute(std::make_tuple(std::cref(values)...), function);

  // Evaluate numerically.
  index_sequence_for(
      [&]<std::size_t I>(std::integral_constant<std::size_t, I>) {
        const auto J_numerical = numerical_jacobian(
            std::get<I>(std::make_tuple(std::cref(values)...)), [&](const auto& x) {
              auto args_perturbed = std::make_tuple(values...);
              std::get<I>(args_perturbed) = x;

              function_evaluator<residual_dim, intrinsic_param_types> eval_perturbed{};
              eval_perturbed.compute_no_jacobians(args_perturbed, function);
              return eval_perturbed.f;
            });
        ASSERT_EIGEN_NEAR(J_numerical, eval.jacobian_array()[I], tol);
      },
      std::make_index_sequence<sizeof...(ArgsValues)>{});
}
...
class ConstrainedNLSTest : public ::testing::Test {
 public:
  ...
  void TestComputeQPCostDerivative() {
    ...
    auto cost_func = [](double x0, double x1, double x2, double x3, double x4,
                        Eigen::Map<Matrix<double, 3, 1>> J0, Eigen::Map<Matrix<double, 3, 1>> J1,
                        Eigen::Map<Matrix<double, 3, 1>> J2, Eigen::Map<Matrix<double, 3, 1>> J3,
                        Eigen::Map<Matrix<double, 3, 1>> J4) -> Matrix<double, 3, 1> {
      // some nonsense
      Vector3d result{x0 * x1 - x3, std::sin(x1) * std::cos(x2), x3 * x0 - x2 * x4};
      if (J0.data()) {
        J0 = Eigen::Vector3d(x1, 0, x3);
        J1 = Eigen::Vector3d(x0, std::cos(x1) * std::cos(x2), 0);
        J2 = Eigen::Vector3d(0, -std::sin(x2) * std::sin(x1), -x4);
        J3 = Eigen::Vector3d(-1, 0, x0);
        J4 = Eigen::Vector3d(0, 0, -x2);
      }
      return result;
    };

    residual cost = make_residual(cost_func, 0, 1, 2, 3, 4);

    // Make up an equality constraint
    auto eq_func = [](double x3, double x4, double x0, Eigen::Map<Matrix<double, 2, 1>> J3,
                      Eigen::Map<Matrix<double, 2, 1>> J4,
                      Eigen::Map<Matrix<double, 2, 1>> J0) -> Vector2d {
      Vector2d result{x3 - x4, -x0 * x0};
      if (J3.data()) {
        J3 = Eigen::Vector2d(1, 0);
        J4 = Eigen::Vector2d(-1, 0);
        J0 = Eigen::Vector2d(0, -2 * x0);
      }
      return result;
    };

    residual eq = make_residual(eq_func, 3, 4, 0);

    // check that the derivatives are correct
    const Vector5 params = (Vector5() << -0.5, 0.2, 0.3, -0.8, 1.2).finished();
    TestResidualFunctionDerivative(tol::kNano, cost_func, params[0], params[1], params[2],
                                   params[3], params[4]);
    TestResidualFunctionDerivative(tol::kNano, eq_func, params[3], params[4], params[0]);
    ...
    const Matrix<double, 1, 1> J_numerical = numerical_jacobian(0.0, [&](const double alpha) {
      ...
      cost.error_vector(params_v.retract(dx * alpha), cost_out.head(3));
      eq.error_vector(params_v.retract(dx * alpha), equality_out.head(2));
    ...
    qp.A_eq.resize(2, 5);
    qp.b_eq.resize(2);
    ConstrainedNonlinearLeastSquares::LinearizeAndFillQP(params_v, params_v, scatter(params_v), 0.0,
                                                         problem, &qp);

    const DirectionalDerivatives J_analytical =
        ConstrainedNonlinearLeastSquares::ComputeQPCostDerivative(qp, dx.head(5));
    ASSERT_NEAR(J_numerical[0], J_analytical.Total(penalty), tol::kNano);
  }
  ...
  static Vector2d Rosenbrock(const Vector2d& params, Eigen::Map<Matrix2d> J_out) {
    constexpr double a = 1.0;
    constexpr double b = 100.0;
    if (J_out.data()) {
      J_out.setZero();
      J_out(0, 0) = -1.0;
      J_out(1, 0) = -2 * params.x() * std::sqrt(b);
      J_out(1, 1) = std::sqrt(b);
    }
  ...
  void TestRosenbrock() {
    TestResidualFunctionDerivative(tol::kNano, &ConstrainedNLSTest::Rosenbrock, Vector2d{5, -3});
    TestResidualFunctionDerivative(tol::kNano, &ConstrainedNLSTest::Rosenbrock, Vector2d{1, 1});
    ASSERT_EIGEN_NEAR(Vector2d::Zero(), Rosenbrock({1, 1}, Eigen::Map<Matrix2d>(nullptr, 2, 2)),
                      tol::kPico);

    // simple problem with only one cost
    Problem problem{};
    problem.costs.push_back(make_residual(&ConstrainedNLSTest::Rosenbrock, 0));
    problem.dimension = 2;
    ...
    p.max_iterations = 5;
    p.max_qp_iterations =
        1;  //  since this is quadratic w/ no constraints, should only need one of these
    ...
    for (const Vector2d& guess : initial_guesses) {
      values guess_v;
      guess_v.insert(0, guess);

      // solve it
      const NLSSolverOutputs outputs = nls.Solve(p, guess_v);
      ...
      // check solution
      ASSERT_EIGEN_NEAR(Vector2d::Ones(), nls.variables().at<Eigen::Vector2d>(0), tol::kMicro)
  ...
  void TestRosenbrockLM() {
    // simple problem with only one cost
    Problem problem{};
    problem.costs.emplace_back(make_residual(&ConstrainedNLSTest::Rosenbrock, 0));
    problem.dimension = 2;
  ...
  void TestInequalityConstrainedRosenbrock() {
    ...
    problem.costs.emplace_back(make_residual(&ConstrainedNLSTest::Rosenbrock, 0));
    problem.dimension = 2;
    problem.inequality_constraints.push_back(var(0, 0) >= 1.2);
    problem.inequality_constraints.push_back(var(0, 1) <= 0.5);
    ...
    for (const Vector2d& guess : initial_guesses) {
      ...
      ASSERT_EIGEN_NEAR(Vector2d(1.2, 0.5), nls.variables().at<Eigen::Vector2d>(0), tol::kMicro)
          << fmt::format("Initial guess: {}\nSummary:\n{}\n",
                         fmt::streamed(guess.transpose().format(test_utils::kNumPyMatrixFmt)),
                         outputs.ToString(true));
    }
  ...
  //       make_residual<10, 6>({0, 1, 2, 3, 4, 5}, &ConstrainedNLSTest::Rosenbrock6D));
  //   problem.dimension = 6;
  ...
  //   Problem problem{};
  //   problem.costs.push_back(make_residual<1, 2>({0, 1}, &ConstrainedNLSTest::Himmelblau1));
  //   problem.costs.push_back(make_residual<1, 2>({0, 1}, &ConstrainedNLSTest::Himmelblau2));
  //   problem.dimension = 2;
  ...
  //     ASSERT_NEAR(0.0, nls.evaluate_nonlinear_error(sol).Total(1.), tol::kMicro);
  //   }
  ...
  //   Problem problem{};
  //   problem.costs.push_back(make_residual<N, N>(index, &ConstrainedNLSTest::SphereFunction<N>));
  //   problem.dimension = N;

  //   problem.equality_constraints.push_back(make_residual<1, 2>(
